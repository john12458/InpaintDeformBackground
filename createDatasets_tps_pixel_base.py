...
def wdgrid2mask(warpped_delta_grid,use_neighbor,use_normlize,norm_factor=8): # 移動8pixel以上即為1
    mask = None
    if use_neighbor:
        ...
        mask = np.amax(np.abs(warpped_delta_grid), axis=-1)[...,np.newaxis]
        if use_normlize:
            mask = mask / norm_factor

    mask = np.clip((1 - mask),0.0,1.0)
    return mask
...
def tps_warp(pillow_img,src_pts,target_pts):
    ...
    dx = (delta_grid[:, :, 0] * (img.shape[1]-1)).astype(np.float32)
    dy = (delta_grid[:, :, 1] * (img.shape[0]-1)).astype(np.float32)
    delta_grid = np.stack([dx,dy],axis=-1)
...
class RandTPS():
    def __init__(self):
        self.warp_f_list=[
            ...
            # p10_8
            # TPSWarp(mesh_size = 48, warp_max_factor=3,num_vertex_wanted_to_move=4,bounding_sample=2,use_normlize=True),
            # TPSWarp(mesh_size = 48, warp_max_factor=3,num_vertex_wanted_to_move=2,bounding_sample=2,use_normlize=True),
            # TPSWarp(mesh_size = 32, warp_max_factor=4,num_vertex_wanted_to_move=4,bounding_sample=3,use_normlize=True),
            # TPSWarp(mesh_size = 32, warp_max_factor=4,num_vertex_wanted_to_move=2,bounding_sample=3,use_normlize=True),


            # p16
            TPSWarp(mesh_size = 32, warp_max_factor=4,num_vertex_wanted_to_move=6,bounding_sample=3,use_normlize=True),
            TPSWarp(mesh_size = 32, warp_max_factor=4,num_vertex_wanted_to_move=4,bounding_sample=3,use_normlize=True),
            TPSWarp(mesh_size = 32, warp_max_factor=4,num_vertex_wanted_to_move=2,bounding_sample=3,use_normlize=True)
    ...
    def __call__(self, pillow_img):
        idx = np.random.randint(len(self.warp_f_list))
...
def sample_one_to_check():
    ...
    axs[0,1].set_title("warpped")
    axs[0,1].imshow(warpped)
    axs[0,2].set_title("mask")
    axs[0,2].imshow(mask ,cmap="gray", vmin=0,vmax=1)

    axs[1,0].set_title("origin * mask")
    axs[1,0].imshow( Image.fromarray(np.uint8(np.array(origin) * mask)))
    axs[1,1].set_title("warpped * mask")
    axs[1,1].imshow( Image.fromarray(np.uint8(np.array(warpped) * mask)))
    # axs[1,2].imshow(np.ones_like(origin)*255)
    axs[1,2].set_title("min_max_mask")
    # norm_factor = math.sqrt(mask.shape[-1]^2 + mask.shape[-2]^2)
    min_max_norm_f = lambda x : (x - x.min()) / (x.max() - x.min())
    min_max_mask = min_max_norm_f(mask)
    axs[1,2].imshow(min_max_mask ,cmap="gray", vmin=0,vmax=1)
